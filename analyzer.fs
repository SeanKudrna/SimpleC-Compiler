...
module analyzer =
  ...
  let rec private vardecl tokens symbolTable = 
    ...
    if next_token = "int" then
      ...
      let varName = (List.head T2).Substring(11)
      let symbolTabEntry = [(varName,"int")] // Pull "identifier:(var)" string and substring after the : to keep only var name

      if (List.contains (varName, "int") symbolTable) || (List.contains (varName, "real") symbolTable) then
        failwith ("redefinition of variable '" + varName + "'")

      let ST2 = List.append symbolTable symbolTabEntry // add symbolTabEntry to end of symbolTable and save new table as ST2
      ...
      let varName = (List.head T2).Substring(11)
      let symbolTabEntry = [(varName,"real")] // Pull "identifier:(var)" string and substring after the : to keep only var name
  ...
  let rec private stmt tokens symbolTable = 
    ...
    if next_token = ";" then
      ...
      (T2, ST2) // symbolTable being updated
